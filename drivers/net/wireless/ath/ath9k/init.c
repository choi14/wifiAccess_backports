		    ...
		    const struct ath_bus_ops *bus_ops)
{
	...
	//jychoi debugfs initialization
	sc->chan_num=40;
	sc->chan_util=0;
	sc->chan_ready=0;
	sc->chan_dir= debugfs_create_dir("jychoi", hw->wiphy->debugfsdir);
	debugfs_create_u8("chan_num", S_IRUSR | S_IWUSR, sc->chan_dir, &sc->chan_num);
	debugfs_create_u8("chan_util", S_IRUSR | S_IWUSR, sc->chan_dir, &sc->chan_util);
	debugfs_create_u8("chan_ready", S_IRUSR | S_IWUSR, sc->chan_dir, &sc->chan_ready);
	//jychoi
