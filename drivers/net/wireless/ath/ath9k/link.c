...
int ath_update_survey_stats(struct ath_softc *sc)
{
	...
	unsigned long rxBusy = 0;
	int instCU = 0;
	int cumCU = 0;
	u8 ACOR = 0;
	unsigned long long sTime = 0;
	unsigned long long sBusy = 0;
	//#jychoi//	
	...
	cycles = (unsigned long) cc->cycles; 
	rxBusy = (unsigned long) cc->rx_busy;   
	instCU =  (int)(rxBusy*255/cycles);
	ACOR = (u8) 255 - instCU;
	sTime = (unsigned long long) survey->time;
	sBusy = (unsigned long long) survey->time_busy;
	cumCU = (int)(sBusy*255/sTime);

	printk("#jychoi# survey_stats || instCU: %d ACOR: %d cycles: %lu rxBusy: %lu \n",
			instCU, (int)ACOR, cycles, rxBusy);
	//printk("#jychoi# survey_stats || cumCU: %d survey->time: %llu survey->time_busy: %llu \n",
	//		cumCU, sTime, sBusy);
	//#jychoi//

	memset(cc, 0, sizeof(*cc));
	ACOR = ACOR & 0xff;
	//jychoi update debugfs
	if(sc->chan_ready==1)
	{
		printk("chan_ready is already set to 1\n");
		sc->chan_util=ACOR;
	}
	else
	{
